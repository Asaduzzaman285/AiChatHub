...
return [
    ...
    // this service. That silently crashes `queue:work` on startup (it checks the cache
    // for a queue:restart signal before pulling its first job) with no clear error
    // visible unless run in the foreground — same bug that broke ai-gateway-service's
    // queue worker before its config/cache.php was added.
    ...
    'prefix' => env('CACHE_PREFIX', 'aichathub_subscription_cache_'),
